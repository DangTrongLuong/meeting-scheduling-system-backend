...
public class CallService {
        ...
        public CallSessionResponse initiateCall(InitiateCallRequest request, UUID initiatorId) {
                ...
                if (!meeting.getStartTime().toLocalDate().equals(today)) {
                        throw new AppException(ErrorStatus.INVALID_INPUT,
                                        "Can only initiate calls for today's meetings");
                }

                // Kiểm tra thời gian hiện tại có gần thời gian bắt đầu meeting không (cho phép
                // 15 phút trước)
                LocalDateTime now = LocalDateTime.now();
                if (now.isBefore(meeting.getStartTime().minusMinutes(15))) {
                        throw new AppException(ErrorStatus.INVALID_INPUT,
                                        "Call can only be initiated 15 minutes before meeting start time");
                }
                ...
                List<MeetingParticipant> meetingParticipants = meetingParticipantRepository
        ...
        public CallSessionResponse getActiveCall(String meetingId) {
                var callSession = callSessionRepository.findActiveCallByMeeting(meetingId)
                                .orElseThrow(() -> new AppException(ErrorStatus.CALL_ACTIVE_NOT_FOUND,
                                                "No active call for this meeting"));
        ...
        public void acceptCall(UUID callId, UUID userId) {
                ...
                CallParticipant participant = callParticipantRepository.findByCallSessionIdAndUserId(callId, userId)
                                .orElseThrow(() -> new AppException(ErrorStatus.PARTICIPANT_NOT_FOUND,
                                                "Participant not found in call"));
